...
void solve()
{
    string s;
    string t;
    ll sa = 0, sb = 0, ta = 0, tb =0;
    cin>>s;
    cin>>t;
    if((ll)s.length() < (ll)t.length()) swap(s,t);

    for(auto c:s) {if(c=='a') ++sa; else ++sb;}
    for(auto c: t) {if(c=='a') ++ta; else ++tb;}

    if((ll)s.length() % (ll)t.length() != 0) { if((sa==(ll)s.length() && ta==(ll)t.length()) || (sb==(ll)s.length() && tb==(ll)t.length())) {cout<<(s+=t)<<endl; return;} else {cout<<-1<<endl; return; } }
    int si = 0, ti = 0;
    while(si<(ll)s.length())
    {
        if(s[si]!=t[ti%(ll)t.length()]) {cout<< -1<<endl; return;}
        ++si,++ti;
    }
    cout<<s<<endl;
}
